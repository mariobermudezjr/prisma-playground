...
class Photon {
    constructor(options = {}) {
        ...
        this.engine = new runtime_1.Engine({
            ...
            datasources,
            generator: { "name": "photon", "provider": "photonjs", "output": "/Users/macbookpro/Desktop/learning/prisma/prisma-playground/node_modules/@generated/photon", "platforms": [], "pinnedPlatform": null, "config": {} },
            platform: undefined
        });
    ...
    get todos() {
        return TodoDelegate(this.dmmf, this.fetcher);
    }
}
...
class UserClient {
    ...
    todos(args) {
        const prefix = this._path.includes('select') ? 'select' : this._path.includes('include') ? 'include' : 'select';
        const path = [...this._path, prefix, 'todos'];
        const newArgs = runtime_1.deepSet(this._args, path, args || true);
        this._isList = true;
        return new TodoClient(this._dmmf, this._fetcher, this._queryType, this._rootField, this._clientMethod, newArgs, path, this._isList);
    }
    get _document() {
...
class PostClient {
    ...
    then(onfulfilled, onrejected) {
        if (!this._requestPromise) {
            this._requestPromise = this._fetcher.request(this._document, this._path, this._rootField, 'Post', this._isList);
        }
        return this._requestPromise.then(onfulfilled, onrejected);
    }
    /**
...
function TodoDelegate(dmmf, fetcher) {
    const Todo = (args) => new TodoClient(dmmf, fetcher, 'query', 'findManyTodo', 'todos', args, []);
    Todo.findOne = (args) => args.select ? new TodoClient(dmmf, fetcher, 'query', 'findOneTodo', 'todos.findOne', args, []) : new TodoClient(dmmf, fetcher, 'query', 'findOneTodo', 'todos.findOne', args, []);
    Todo.findMany = (args) => new TodoClient(dmmf, fetcher, 'query', 'findManyTodo', 'todos.findMany', args, []);
    Todo.create = (args) => args.select ? new TodoClient(dmmf, fetcher, 'mutation', 'createOneTodo', 'todos.create', args, []) : new TodoClient(dmmf, fetcher, 'mutation', 'createOneTodo', 'todos.create', args, []);
    Todo.delete = (args) => args.select ? new TodoClient(dmmf, fetcher, 'mutation', 'deleteOneTodo', 'todos.delete', args, []) : new TodoClient(dmmf, fetcher, 'mutation', 'deleteOneTodo', 'todos.delete', args, []);
    Todo.update = (args) => args.select ? new TodoClient(dmmf, fetcher, 'mutation', 'updateOneTodo', 'todos.update', args, []) : new TodoClient(dmmf, fetcher, 'mutation', 'updateOneTodo', 'todos.update', args, []);
    Todo.deleteMany = (args) => args.select ? new TodoClient(dmmf, fetcher, 'mutation', 'deleteManyTodo', 'todos.deleteMany', args, []) : new TodoClient(dmmf, fetcher, 'mutation', 'deleteManyTodo', 'todos.deleteMany', args, []);
    Todo.updateMany = (args) => args.select ? new TodoClient(dmmf, fetcher, 'mutation', 'updateManyTodo', 'todos.updateMany', args, []) : new TodoClient(dmmf, fetcher, 'mutation', 'updateManyTodo', 'todos.updateMany', args, []);
    Todo.upsert = (args) => args.select ? new TodoClient(dmmf, fetcher, 'mutation', 'upsertOneTodo', 'todos.upsert', args, []) : new TodoClient(dmmf, fetcher, 'mutation', 'upsertOneTodo', 'todos.upsert', args, []);
    return Todo; // any needed until https://github.com/microsoft/TypeScript/issues/31335 is resolved
}
class TodoClient {
    constructor(_dmmf, _fetcher, _queryType, _rootField, _clientMethod, _args, _path, _isList = false) {
        this._dmmf = _dmmf;
    ...
    then(onfulfilled, onrejected) {
        if (!this._requestPromise) {
            this._requestPromise = this._fetcher.request(this._document, this._path, this._rootField, 'Todo', this._isList);
        }
        return this._requestPromise.then(onfulfilled, onrejected);
    }
    /**
